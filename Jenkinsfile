ipipeline {
    ...
    environment {
        ...
        AWSECR = '178020491921.dkr.ecr.ap-northeast-2.amazonaws.com/personalize'
        REGION = 'ap-northeast-2'
    }
    stages {
        stage('Checkout Github') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: '*/personalize']], extensions: [],
        ...
        stage('EKS manifest file update') {
            steps {
                ...
                sh "sed -i 's@${AWSECR}:.*@${AWSECR}:${currentBuild.number}@g' personalize/personalize.yaml"
